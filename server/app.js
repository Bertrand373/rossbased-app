...
const tttvRoutes = require('./routes/tttvRoutes');
...
const oracleThreadRoutes = require('./routes/oracleThreadRoutes');
const oracleTranscribeRoutes = require('./routes/oracleTranscribeRoutes');
const { apiRouter: oracleShareApi, pageRouter: oracleSharePage } = require('./routes/oracleShareRoutes');
const oraclePulseRoutes = require('./routes/oraclePulseRoutes');
const oraclePinRoutes = require('./routes/oraclePinRoutes');
const oracleNotesRoutes = require('./routes/oracleNotesRoutes');
...
app.post('/api/ai/chat/stream', authenticate, oracleLimiter, async (req, res) => {
  ...
  try {
    ...
    const hasActiveStripeSub = !!user.subscription?.stripeSubscriptionId && 
    ...
    } else if (!isBetaPeriod && userHasPremium && !isAscended && !isPureOG && currentCount >= PREMIUM_DAILY_LIMIT) {
      return res.status(429).json({ 
        error: 'Daily limit reached',
        message: `You've used all ${PREMIUM_DAILY_LIMIT} daily messages. Resets at midnight.`
      });
    } else if (!isBetaPeriod && isPureOG && combinedCount >= GRANDFATHERED_DAILY_LIMIT) {
      return res.status(429).json({ 
        error: 'Daily limit reached',
    ...
    if (isAdminUser || hasFreeAccess) {
      ...
      userTier = 'ascended';
    } else if (userHasPremium && !isPureOG) {
...
app.use('/api/tttv', authenticate, tttvRoutes);
...
app.use('/api/oracle/threads', authenticate, oracleThreadRoutes);
app.use('/api/oracle/transcribe', authenticate, oracleTranscribeRoutes);
...
app.use('/api/oracle/notes', authenticate, oracleNotesRoutes);
...
app.use('/api/circles', authenticate, circleRoutes);
